...
// this class refuses (never forwards to ServoOutput at all) a pulse that
...
public:
    ...
    // underlying ServoOutput at all) if hasServoIndex is true and pulseUs
    // falls outside that servo's recorded min/max -- reject, not clamp,
    // so the operator gets a clear ack/log line rather than a silently
    // truncated value with no feedback (see the design discussion this
    // was built from). reason is set to a static string on refusal.
    bool commandPulse(uint8_t board, uint8_t channel, uint16_t pulseUs, bool hasServoIndex,
                       uint8_t servoIndex, const char** reason);
