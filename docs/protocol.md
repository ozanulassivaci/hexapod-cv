...
| `stack_free_bytes` | yes, nullable | Free stack on the robot's link task at its deepest point since its boot (`uxTaskGetStackHighWaterMark`, which returns bytes on ESP-IDF). `null` from both mocks, which have no comparable stack — a fabricated number there would read as a warning. Present because a stack overflow on this firmware is not a graceful failure: it reboots the board mid-packet, which from the PC is indistinguishable from a dead link. It has happened once, and cost a session. |
